pipeline
{
	...
	stages{
		stage('Code Checkout'){
			steps{
				git branch: 'main', url: 'https://github.com/GiriDevOps01/Rigstration-Form'
		...
		stage('Deploy War into Tomcat'){
		    steps{
		       sshagent(['Tomcat-Web-Server']) {
	            sh 'scp /root/.jenkins/workspace/Registration-Form-1/webapp/target/*.war root@192.168.172.128/:/opt/tomcat/webapps' 
	            }
		    }
		}
...
''', cc: '', from: '', replyTo: '', subject: 'Jenkins Pipeline Job Execution', to: 'kgiriprasadreddy22@gmail.com'
}
}
